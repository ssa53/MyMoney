...
function renderTransactionList(targetTransactions, targetListEl, targetEmptyStateEl) {
    ...
    const groupedTransactions = targetTransactions.reduce((groups, t) => {
        if (!groups[t.date]) groups[t.date] = [];
        groups[t.date].push(t);
        return groups;
    }, {});
...
// script.js의 이벤트 리스너 섹션에 추가

// ★★★ '지출 내역' 페이지 필터링 컨트롤 이벤트 리스너 ★★★
if (historyPeriodSelect) {
    historyPeriodSelect.addEventListener('change', () => {
        if (historyPeriodSelect.value === 'daily') {
            historyDateInput.style.display = 'block'; // '일별' 선택 시 날짜 입력창 보이기
        } else {
            historyDateInput.style.display = 'none'; // 그 외에는 숨김
        }
    });
}
...
if (filteredTransactionList) {
    filteredTransactionList.addEventListener('click', async (event) => {
    ...
    if (menuItems) {
        menuItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                // 1. 모든 페이지를 숨기고 메뉴 활성화 상태를 제거합니다.
                [assetManagementPage, statisticsPage, transactionHistoryPage, settingsPage].forEach(page => { // ★★★ transactionHistoryPage 추가 ★★★
                ...
                // 3. 페이지별 초기화 및 데이터 렌더링
                if (currentUser) {
                    if (pageId === 'statistics-page') {
                        renderStatistics('monthly');
                    } else if (pageId === 'transaction-history-page') { // ★★★ '지출 내역' 페이지 로직 추가 ★★★
                        // 기본값으로 '월별' 선택 및 오늘 날짜 설정
                        historyPeriodSelect.value = 'monthly';
                        historyDateInput.valueAsDate = new Date(); // 오늘 날짜로 기본 설정
                        historyDateInput.style.display = 'none'; // 월별 선택 시 날짜 입력창 숨김
                        filterAndRenderHistory(); // 기본 조회 실행
                    }
                }
            });
        });
